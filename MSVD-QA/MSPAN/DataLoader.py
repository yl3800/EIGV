...
from utils import load_file, pause
...
class VideoQADataset(Dataset):
    def __init__(self, sample_list_path, feat_path, split, args):
        ...
        with h5py.File(app_feat_path, 'r') as fp:
            ...
            for id, (vid, feat) in enumerate(zip(vids, feats)):
                self.frame_feats[str(vid)] = feat # (16, 2048) get mid frame in each seg
    ...
    def __getitem__(self, idx):
        ...
        video_id, ans_id = str(cur_sample['video_id']), int(cur_sample['answer_id'])
...
if __name__ == "__main__":
    ...
    for sample in train_loader:
        vis_feat, qns, q_lengths, ans=sample
